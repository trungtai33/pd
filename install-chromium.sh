...
echo "deb http://deb.debian.org/debian stable main" > /etc/apt/sources.list.d/debian.sources.list
if [ "$(id -u)" = "0" ]; then
	...
	apt-key del 605C66F00D6C9793
	rm -f /etc/apt/sources.list.d/debian.sources.list
	rm -f /etc/apt/trusted.gpg
	rm -f /etc/apt/trusted.gpg~
	rm -f /etc/apt/trusted.gpg.d/debian-archive*.gpg
else
